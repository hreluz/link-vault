  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end lg:items-center lg:justify-center" onClick={handleClose}>
      ...
      >
        ...
        {/* header */}
        <div className="flex items-start gap-2 border-b border-surface-100 px-5 py-3 lg:py-4 dark:border-surface-800">
          <div className="min-w-0 flex-1">
            <p className="line-clamp-1 text-sm font-semibold text-surface-900 dark:text-surface-50">{link.title}</p>
            <p className="mt-0.5 text-xs text-surface-400 dark:text-surface-500">{link.site_name}</p>
          </div>
          <button
            onClick={handleClose}
            aria-label="Close"
            className="hidden h-7 w-7 shrink-0 items-center justify-center rounded-lg text-surface-400 transition hover:bg-surface-100 hover:text-surface-600 lg:flex dark:text-surface-500 dark:hover:bg-surface-800 dark:hover:text-surface-300"
          >
            ✕
          </button>
        </div>
        ...
        {/* quick actions */}
        <div className="grid grid-cols-3 gap-2 border-b border-surface-100 p-4 dark:border-surface-800">
          <a
            href={link.url}
            target="_blank"
            rel="noopener noreferrer"
            onClick={handleClose}
            className="flex items-center justify-center gap-1.5 rounded-xl border border-surface-200 py-2.5 text-sm font-medium text-surface-600 transition hover:bg-surface-50 dark:border-surface-700 dark:text-surface-400 dark:hover:bg-surface-800"
          >
        ...
        <ul className="py-2">
          <li className="px-5 pb-1 pt-2">
            <p className="text-xs font-semibold uppercase tracking-wide text-surface-400 dark:text-surface-500">Status</p>
          </li>
            ...
            return (
              <li key={s}>
                <button
                  ...
                  className={`flex w-full items-center gap-4 px-5 py-3.5 text-left transition active:bg-surface-100 dark:active:bg-surface-800 ${
                    active ? 'bg-surface-50 dark:bg-surface-800' : 'hover:bg-surface-50 dark:hover:bg-surface-800'
                  }`}
                >
                  <span className="text-lg" aria-hidden="true">{STATUS_ICONS[s]}</span>
                  <span className={`text-sm font-medium ${active ? 'text-primary-600 dark:text-primary-400' : 'text-surface-700 dark:text-surface-300'}`}>
                    {STATUS_CONFIG[s].label}
                  </span>
                  {active && <span className="ml-auto text-sm font-semibold text-primary-600 dark:text-primary-400">✓</span>}
                </button>
              </li>
            )
          })}
          ...
          {confirmingDelete ? (
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="rounded-xl border border-surface-200 px-4 py-2 text-sm font-medium text-surface-600 transition hover:bg-surface-50 dark:border-surface-700 dark:text-surface-400 dark:hover:bg-surface-800"
              >
                Cancel
              </button>
              <button
